...
async function gameSetup(word) {
  ...
  await getAnagrams(word).then((results) => {
    ...
    validWords.forEach((word) => {
      ...
      button.title = `${word.length}-letter word. Click to reveal!
...
(Purchased!)`;
          ...
          let totalCoinsEarned =
            parseInt(localStorage.getItem('coinsEarnedWordscapes')) || 0;
          localStorage.setItem(
            'coinsEarnedWordscapes',
            totalCoinsEarned - word.length + 1
          );
          document.querySelector('.awardCounter').textContent =
            totalCoinsEarned;
...
document
  ...
  .addEventListener('click', () => randomize(points));
...
function drawCirclePoints(word, radius, center) {
  ...
  for (let i = 0; i < pts; i++) {
    ...
    document.querySelector(
      '#gameArea'
    ).innerHTML += `<g class="letter" data-letter="${word[
      i
    ].toUpperCase()}" data-letter-id="${word[
      i
    ].toUpperCase()}${getUUID()}" transform="translate(${newX * 1}, ${
      newY * 1
    })">
            <circle class="letterCircle" cx="0" cy="0" r="11" fill="rgba(255,255,255,0.5)" />
            <text class="letterText" x="0" y="2" font-size="16" text-anchor="middle" dominant-baseline="middle">${word[
              i
            ].toUpperCase()}</text>
        </g>`;
  }
...
function startDrawingPath(letter) {
  ...
  document
    ...
    .setAttribute('d', `M ${startX} ${startY}`);
  document
    .querySelector('.connectedLetters')
    .setAttribute('d', `M ${startX} ${startY}`);
}
...
function updatePath(letter, event) {
  ...
  if (letter) {
    ...
    path =
      path.split('L')[0] + ` L ${(cx * 1).toFixed(3)} ${(cy * 1).toFixed(3)}`;
